...
- **Ordenamiento por Arrastrar y Soltar (Julio de 2026):** Se integró la biblioteca `vuedraggable` en el panel de administrador para permitir organizar el orden de visualización de las tiendas y sus respectivos productos de forma interactiva (drag & drop). Se agregó una nueva columna `orden` en la base de datos a las tablas `tiendas` y `products`. Este orden personalizado se refleja automáticamente tanto en el Megamenú de navegación, como en el Home y en el catálogo interno de cada tienda en la vista del cliente.
- **Fix Crítico — Procesamiento de Pagos Mercado Pago y Diagnóstico de Fallos (24 de Julio de 2026):**
  - **Problema:** Todos los pedidos de la plataforma se marcaban como `"Fallido"` al intentar pagar, impidiendo la entrada de nuevas compras.
  - **Diagnóstico mediante Logs:** Se utilizó el sistema de logging de la columna `motivo_fallo` en la tabla `pedidos` (implementado en `create_motivo_fallo_column.js` y visualizable en el panel `admin` en `PedidoDetalle.vue`). Al consultar las órdenes fallidas en BD, se descubrió la respuesta exacta de Mercado Pago: `"Error técnico: The name of the following parameters is wrong : [additional_info.payer.address.city]"`.
